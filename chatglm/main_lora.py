...
class LoRASeq2SeqTrainer(Seq2SeqTrainer):
    def create_optimizer_and_scheduler(self, num_training_steps):
        logger.warning("Set get_polynomial_decay_schedule_with_warmup power 2 for AdamW")
...
def main():
    ...
    data_collator = DataCollatorForSeq2Seq(
        ...
        pad_to_multiple_of=None, # if peft_args.lora_rank is not None else None,
        padding=False
    )
    ...
    trainer = LoRASeq2SeqTrainer(
        ...
        save_prefixencoder=peft_args.lora_rank is not None #是否只保存训练的参数
    )
